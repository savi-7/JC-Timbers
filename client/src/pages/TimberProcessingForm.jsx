...
export default function TimberProcessingForm() {
  ...
  // Derive estimated cost from cubic feet
  // Time is used only for admin-side planning; customers see cost only.
  const processingHours = totalCubicFeet > 0 ? Math.ceil(totalCubicFeet / 10) : 0;
  const ratePerHour = 1200; // ₹1200 per hour
  const ratePerCubicFoot = ratePerHour / 10; // ₹120 per cubic foot
  const estimatedCost = totalCubicFeet * ratePerCubicFoot;
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200">
            <form onSubmit={handleSubmit} className="p-6 lg:p-8 space-y-6">
              ...
              {/* Estimated Cost based on cubic feet */}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-600 mt-1">
                      Pricing is ₹{ratePerCubicFoot.toLocaleString("en-IN")} per cubic foot (10 cubic feet = ₹
                      {ratePerHour.toLocaleString("en-IN")}).
                    </p>
                  </div>
                </div>
              </div>
